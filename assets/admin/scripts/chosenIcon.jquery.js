    ...
    $.fn.chosenIcon = function (options) {
        ...
        return this.each(function () {
            ...
            $select.change(function () {
                var iconClassName = ($select.find('option:selected').attr('value'))
                    ? $select.find('option:selected').attr('value') : null;

                if (iconClassName) {
                    var iconContent = $.getCSSValue('.' + iconClassName, 'content', ':before');
                    $chosen.find('.chosen-single span').attr('value', iconContent);
                }
